...
## Bump infra.ado without cutting an EE release

When a new `infra.ado` GitHub Release exists, you do not have to edit `requirements.yml` by hand.

1. **Actions** → **Bump infra.ado collection** → **Run workflow**.
...
| [`.github/workflows/bump-infra-ado.yml`](../.github/workflows/bump-infra-ado.yml) | Open a PR to pin a new infra.ado release |
